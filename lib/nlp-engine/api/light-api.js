...
module.exports = {
    changeColor: (device, color, classification, cb) => {
        if (!classification.isCheckCommand) {
            url = "http://localhost:4030/connect/account/" + accountId + "/philips/device/" + device.id + "/action/setColor";
            var options = {
                url: url,
                json: true,
                body: { value: color }
            ...
            url = "http://localhost:4030/connect/account/" + accountId + "/philips/device/" + device.id;
            var options = {
                url: url,
                json: true
            }
    ...
    turnOn: (device, classification, cb) => {
        if (!classification.isCheckCommand) {
            url = "http://localhost:4030/connect/account/" + accountId + "/philips/device/" + device.id + "/action/setSwitch";
            var options = {
                url: url,
                json: true,
                body: { value: "ON" }
    ...
    getAll: (cb) => {
        url = "http://localhost:4030/connect/account/" + accountId + "/philips/devices/getAllLights";
        var options = {
            url: url
        }
        request.get(options, function (err, resp) {
